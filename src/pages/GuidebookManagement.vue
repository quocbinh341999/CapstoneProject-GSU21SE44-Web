<template>
  <div>
    ...
    >
    <el-dialog title="Thêm cẩm nang" :visible.sync="dialogFormAddVisible">
...
export default {
  ...
  methods: {
    ...
    async confirm(index, row) {
      ...
      let GuidebookId = this.tableData[this.editedIndex].id;
      try {
    ...
    async confirmAdd() {
      ...
      try {
        if (this.addNews.imageFile) {
          ...
          await axios.post(
            `http://mumbicapstone-dev.ap-southeast-1.elasticbeanstalk.com/api/Guidebooks/AddGuidebook`,
            {
              title: titleNews,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === "confirm") {
            ...
            axios
              .put(
                `http://mumbicapstone-dev.ap-southeast-1.elasticbeanstalk.com/api/Guidebooks/DeleteGuidebook/` +
                  this.NewsIdDelete
              )
              .then((response) => {});
            setTimeout(() => {
